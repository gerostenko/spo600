...
// --------------------------------------------------------------------
	...
	// Q: should we use 32767 or 32768 in next line? why? With both of them we loose some precision
	vol_int = (int16_t) (0.75 * 327678.0);
	...
	while ( in_cursor < limit ) {
		__asm__ (
			...
			// two lines? Why? It fails to compile, since we refer to our variables with the alias on in and out. We need to describe it in optional input and ouput descriptors
			: [in]"+r"(in_cursor)
...
// --------------------------------------------------------------------
	...
	// Q: are the results usable? are they correct? More or less. 
	printf("Result: %d\n", ttl);
